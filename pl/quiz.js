document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadStep() {
        ...
        if (currentStep === 1) { // Шаг 2
            ...
            sliderLabel.textContent = "Twój budżet na zajęcia:";
            ...
            sliderInput.max = "98";
            sliderInput.step = "2";
            sliderInput.value = userAnswers[currentStep] || "82"; // Используем сохраненное значение или 200 по умолчанию
            const valueDisplay = document.createElement("span");
            valueDisplay.classList.add("value-display");
            valueDisplay.textContent = `${sliderInput.value} zł`;
        ...
        } else if (currentStep === 8) { // Шаг 9 (индекс 8)
            optionsContainer.classList.add("step-9");
            ...
            // Обновляем стили кнопки после добавления формы
            updateButtonStyles();
        } else {
            // Обработка других шагов (радио или чекбоксы)
            if (stepData.options) {
    ...
    function saveAnswer() {
        if (currentStep === 5) { // Шаг 6 (чекбоксы дней и времени)
            ...
            console.log("Данные из формы сохранены:", quizResponses[currentStep]); // Добавляем логирование
    ...
    function updateButtonStyles() {
        ...
        if (currentStep === 9) { // Последний шаг
            const nameInput = document.getElementById("input-text");
            const phoneInput = document.getElementById("input-tel");
            hasSelection = nameInput && phoneInput && nameInput.value.trim() !== "" && phoneInput.value.trim() !== "";
            // Удаляем предыдущий обработчик события, если он был добавлен ранее
            nextButton.removeEventListener("click", submitQuiz);
            // Добавляем обработчик события только на последнем шаге
            if (hasSelection) {
                nextButton.addEventListener("click", submitQuiz);
            }
        } else if (currentStep === 5) { // Шаг 6 (чекбоксы)
